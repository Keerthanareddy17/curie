...
// curie deviation from frontend/legacy/Molecular3DVisualizer.tsx (the pristine
// vendored reference — diff this file against it to see the changes): added
// `structureInfo`, an optional override for the summary metrics panel below
// the viewer. Without it, that panel's `metrics` state defaults to an honest
// "Unavailable" (see the `useState` a few lines down), only ever replaced by
// this component's OWN browser-side ESMFold fetch, triggered by the
// `sequence` prop. curie's backend already does that fold server-side and
// hands back a real PDB via `pdbData`, so this component is deliberately
// never given `sequence` here — StructureViewer.tsx always supplies
// `structureInfo` with the real backend-computed numbers instead.
...
interface Molecular3DVisualizerProps {
  ...
  structureInfo?: StructureInfo; // curie addition — see comment above
}
...
export default function Molecular3DVisualizer({
  ...
  // Computed metrics for display. Only used when the caller does not supply
  // `structureInfo` (curie's StructureViewer always does, from the real
  // backend-computed structure) — see the module comment above.
  ...
  // Placeholder-model metrics reset: only relevant when neither `structureInfo`
  // nor a real fold result is available (see the module comment above — curie's
  // StructureViewer always supplies `structureInfo` from the real backend fold).
  useEffect(() => {
    if (mode === "dna") {
      setMetrics({
        residues: 0,
  ...
  useEffect(() => {
    ...
    try {
      const v = $3Dmol.createViewer(parent, {
        backgroundColor: "#0b0e13" // matches curie's dark theme (see index.css body background)
      });
      setViewer(v);
  ...
  return (
    <div
      ref={containerRef}
      className="relative flex flex-col items-center justify-between bg-panel border border-line rounded-2xl p-4 select-none w-full min-h-[440px]"
    >
      {/* Header: mode label + interaction controls */}
      <div className="w-full flex items-center justify-between border-b border-line pb-2.5 mb-2">
        <div>
          <span className="text-[10px] font-mono tracking-wider text-teal uppercase font-bold">
            {mode === "dna" ? "Nucleic acid structure" : "Predicted protein structure"}
          </span>
          {title && (
            <h4 className="text-xs font-semibold text-ink tracking-tight leading-none mt-1 flex items-center gap-1.5">
              {title}
              {mode === "protein" && foldingStatus === "success" && (
                <span className="inline-flex items-center gap-0.5 text-[8.5px] font-mono font-bold text-emerald-400 bg-emerald-500/10 px-1 py-0.5 rounded border border-emerald-500/30">
                  <Sparkles className="w-2.5 h-2.5" /> Folded
                </span>
              )}
            </h4>
          )}
        ...
        <div className="flex gap-1 bg-panel-light px-2 py-1 rounded-lg border border-line shrink-0">
          <button
            onClick={handleToggleRotation}
            className="p-1 rounded-md text-muted-ink hover:text-ink hover:bg-line/60 transition-colors cursor-pointer"
            title={isRotating ? "Pause rotation" : "Auto revolve"}
          >
            {isRotating ? <Pause className="w-3.5 h-3.5 text-teal" /> : <Play className="w-3.5 h-3.5 text-muted-ink" />}
          </button>
          <button
      ...
      {/* 3D viewport */}
      <div className="relative w-full flex-1 flex items-center justify-center min-h-[260px] overflow-hidden bg-[#0b0e13] rounded-xl border border-line-strong">

        {foldingStatus === "folding" && (
          <div className="absolute inset-x-0 top-1/2 -translate-y-1/2 z-20 mx-auto px-6 py-4 bg-panel/95 border border-teal/35 rounded-xl max-w-sm text-center shadow-md backdrop-blur-md space-y-3">
            <RefreshCw className="w-6 h-6 animate-spin text-teal mx-auto" />
            <div>
              <h5 className="font-bold text-xs text-ink uppercase tracking-wider">Folding structure</h5>
              <p className="text-[10px] text-muted-ink mt-1">Calling the ESM Atlas structure prediction API...</p>
            </div>
          </div>
        )}
        ...
        {hasWebGL && scriptsLoaded ? (
          ...
          <div className="relative w-full h-[260px] flex items-center justify-center">
            ...
            />
            <div className="absolute top-2 left-2 inline-flex items-center gap-1 text-[8.5px] font-mono uppercase bg-amber/10 text-amber border border-amber/30 backdrop-blur-xs px-2 py-0.5 rounded">
              <AlertCircle className="w-2.5 h-2.5" /> WebGL unavailable — 2D fallback
            </div>
          </div>
        )}

        <div className="absolute bottom-2.5 right-2 text-[9px] font-mono text-muted-ink bg-panel/80 border border-line px-2 py-0.5 rounded backdrop-blur-xs">
          Drag to rotate • Scroll/Pinch
        </div>
      </div>

      {/* Structure metrics */}
      <div className="w-full mt-3 bg-panel-light border border-line rounded-xl p-2.5 grid grid-cols-2 md:grid-cols-4 gap-2.5 text-left text-xs font-mono">
        <div className="bg-panel p-2 rounded-lg border border-line/80 leading-tight">
          <span className="text-[8.5px] text-muted-ink uppercase block tracking-wider mb-0.5">Residues</span>
          <span className="text-[11.5px] font-bold text-ink">{(structureInfo ?? metrics).residues}</span>
        </div>
        <div className="bg-panel p-2 rounded-lg border border-line/80 leading-tight">
          <span className="text-[8.5px] text-muted-ink uppercase block tracking-wider mb-0.5">Atoms</span>
          <span className="text-[11.5px] font-bold text-ink">{(structureInfo ?? metrics).atoms}</span>
        </div>
        <div className="bg-panel p-2 rounded-lg border border-line/80 leading-tight">
          <span className="text-[8.5px] text-muted-ink uppercase block tracking-wider mb-0.5">Mean pLDDT</span>
          <span className="text-[11.5px] font-bold text-teal flex items-center gap-1">
            {(structureInfo ?? metrics).confidence}
          </span>
        </div>
        <div className="bg-panel p-2 rounded-lg border border-line/80 leading-tight">
          <span className="text-[8.5px] text-muted-ink uppercase block tracking-wider mb-0.5">Source</span>
          <span className="text-[10px] font-bold text-ink truncate block" title={(structureInfo ?? metrics).source}>
            {(structureInfo ?? metrics).source}
          </span>
        </div>
      </div>

      {mode === "protein" && activeSegment && (
        <div className="w-full mt-2.5 px-3 py-2 rounded-xl bg-panel-light border border-line text-[10px] font-mono text-muted-ink flex justify-between items-center tracking-tight leading-normal">
